...
import type { TimelineInstance, TimelineStep, TimelineMessage } from '@/lib/types/timeline'
 ...
 * Mostra TODAS as fases do template: concluídas marcadas, a atual expandida com
 * mensagens/campos/ações, futuras esmaecidas. Sinaliza de quem é a vez
 * (cliente vs. agência) e oferece o lembrete pronto quando a bola está com o
 * cliente. Substitui a renderização compacta da aba (que era de dashboard).
 */
export function OnboardingTimeline({ instance, cliente, onStepComplete, onStepWait }: Props) {
  const steps = useMemo(
...
function StepRow({
  step, estado, isLast, vars, cliente, onComplete, onWait,
}: {
  step: TimelineStep
  estado: EstadoStep
  isLast: boolean
  ...
  return (
    <div className="flex gap-[0.875rem]">
      ...
      {/* Conteúdo */}
      <div className={cn('flex-1 pb-[1.25rem]', estado === 'futuro' && 'opacity-55')}>
        ...
        {step.content && aberto && (
          <p className="text-ink-muted text-[0.8125rem] mt-[0.375rem]">{step.content}</p>
        )}

        {/* Detalhe só na etapa atual */}
        {aberto && (
