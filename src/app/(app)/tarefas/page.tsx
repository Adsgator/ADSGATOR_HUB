...
import {
  ...
  Tag, LayoutList, LayoutGrid, X,
} from 'lucide-react'
import { fadeScale, slideInBottom } from '@/lib/motion'
...
const KANBAN_COLS = [
  { id: 'pendente',     label: 'Pendente',     color: 'text-status-blue',  dot: 'bg-status-blue'  },
  { id: 'em_andamento', label: 'Em Andamento', color: 'text-ads-500',      dot: 'bg-ads-500'      },
  { id: 'feito',        label: 'Concluído',    color: 'text-status-green', dot: 'bg-status-green' },
]
...
function TarefaAccordion({
  t, expanded, onToggle, onConcluir, onAdiar, onDeletar, onEditar, selected, onSelect,
}: {
  t: TarefaComCliente
  expanded: boolean
  onToggle: () => void
  ...
  return (
    <ContextMenu items={ctxItems}>
      <div className={cn(
        ...
        selected && 'ring-1 ring-ads-500/40',
      )}>
        ...
        >
          ...
          >
            <input
              ...
              className="w-[1rem] h-[1rem] accent-ads-500 cursor-pointer"
            />
          </span>
          ...
          <div className="px-[1rem] pb-[1rem] border-t border-surface-border animate-fade-up">
            ...
            <div className="flex items-center gap-[0.5rem] mt-[0.875rem]">
              ...
              <div className="relative group/adiar">
                ...
                <div className="absolute left-0 top-full mt-[0.25rem] bg-surface-elevated border border-surface-border rounded-[0.5rem] shadow-xl z-50 min-w-[8rem] hidden group-hover/adiar:block group-hover/adiar:pointer-events-auto pointer-events-none animate-fade-scale origin-top overflow-visible">
                  {[
...
/* ─── Kanban Card ────────────────────────────────────────── */
function KanbanCard({ t, onEditar }: { t: TarefaComCliente; onEditar: () => void }) {
  const prio = PRIO_CONFIG[t.prioridade]
  const atrasada = (t.data_prazo?.slice(0, 10) ?? '') < hojeSt() && t.status !== 'feito'
  ...
  return (
    <div
      onClick={onEditar}
      className="bg-surface-card border border-surface-border rounded-xl p-[0.875rem] cursor-pointer hover:border-ads-500/50 transition-colors"
    >
      <p className="text-ink-primary text-[0.875rem] font-medium mb-[0.5rem] leading-snug">{t.titulo}</p>
      <div className="flex items-center gap-[0.375rem] flex-wrap">
        <span className={cn(
          'inline-flex items-center gap-[0.1875rem] text-[0.625rem] font-semibold px-[0.375rem] py-[0.0625rem] rounded-full bg-surface-hover',
          prio.color,
        )}>
          <span className={cn('w-[0.3125rem] h-[0.3125rem] rounded-full', prio.dot)} />
          {prio.label}
      ...
      <div className="flex items-center gap-[0.5rem] mt-[0.5rem] flex-wrap">
        {t.data_prazo && (
          <span className={cn('flex items-center gap-[0.1875rem] text-[0.6875rem]', atrasada ? 'text-status-red font-semibold' : 'text-ink-muted')}>
            <Clock className="w-[0.5625rem] h-[0.5625rem]" strokeWidth={2} />
            {new Date(t.data_prazo).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}
          </span>
        )}
        {t.cliente_nome && (
          <span className="flex items-center gap-[0.1875rem] text-[0.6875rem] text-ink-muted">
            <User className="w-[0.5625rem] h-[0.5625rem]" strokeWidth={2} />
            {t.cliente_nome}
          </span>
        )}
...
export default function TarefasPage() {
  ...
  const [kanbanTarefas, setKanbanTarefas] = useState<TarefaComCliente[]>([])
  ...
  }, [viewMode, carregar, carregarKanban])
  ...
  return (
        ...
        <div className="flex items-center gap-[0.5rem]">
          ...
          <div className="flex items-center rounded-lg overflow-hidden border border-surface-border">
            <Tooltip content="Lista" side="bottom">
              <button
                ...
                className={cn(
                  'flex items-center justify-center w-[2rem] h-[2rem] transition-colors',
                  viewMode === 'list' ? 'bg-ads-500 text-white' : 'bg-surface-hover text-ink-secondary hover:text-ink-primary',
                )}
              >
                <LayoutList className="w-[0.875rem] h-[0.875rem]" strokeWidth={1.75} />
              </button>
            </Tooltip>
            <Tooltip content="Kanban" side="bottom">
              <button
                ...
                className={cn(
                  'flex items-center justify-center w-[2rem] h-[2rem] transition-colors',
                  viewMode === 'kanban' ? 'bg-ads-500 text-white' : 'bg-surface-hover text-ink-secondary hover:text-ink-primary',
                )}
              >
          ...
          <Tooltip content="Atualizar" side="bottom">
            <Button
              variant="secondary"
              size="sm"
              onClick={viewMode === 'kanban' ? carregarKanban : carregar}
    ...
    >
      <div className="page-enter">
          ...
          <DragDropContext onDragEnd={handleDragEnd}>
            {loading ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-[1rem]">
                {KANBAN_COLS.map((col) => (
                  <div key={col.id} className="bg-surface-hover/50 rounded-2xl p-[1rem] min-h-[20rem]">
                    <div className="h-[1.25rem] w-[7rem] rounded-full skeleton-shimmer mb-[1rem]" />
                    <div className="flex flex-col gap-[0.5rem]">
                      {[...Array(3)].map((_, i) => (
                        <div key={i} className="h-[5rem] rounded-xl skeleton-shimmer" />
                      ))}
                    </div>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-[1rem]">
                {KANBAN_COLS.map((col) => {
                  const colItems = kanbanTarefas.filter((t) => t.status === col.id)
                  return (
                    <div key={col.id} className="bg-surface-hover/50 rounded-2xl p-[1rem] min-h-[20rem]">
                      <div className="flex items-center gap-[0.5rem] mb-[0.875rem]">
                        <span className={cn('w-[0.5rem] h-[0.5rem] rounded-full', col.dot)} />
                        <span className={cn('text-[0.875rem] font-semibold', col.color)}>{col.label}</span>
                        <span className="ml-auto text-[0.6875rem] text-ink-muted bg-surface-card border border-surface-border px-[0.375rem] py-[0.0625rem] rounded-full">
                          {colItems.length}
                        </span>
                      </div>
                      <Droppable droppableId={col.id}>
                        {(provided, snapshot) => (
                          <div
                            ...
                            className="flex flex-col gap-[0.5rem] min-h-[12rem] rounded-xl"
                            style={{
                              backgroundColor: snapshot.isDraggingOver ? 'var(--surface-hover)' : 'transparent',
                              padding: snapshot.isDraggingOver ? '0.375rem' : '0',
                              transition: 'all 0.2s',
                            }}
                          >
                            {colItems.map((t, idx) => (
        ...
        {/* ══ LIST VIEW ══════════════════════════════════════════ */}
        {viewMode === 'list' && (
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_18rem] gap-[1.5rem] items-start">
            ...
            <div className="flex flex-col gap-[1rem] sticky top-[1.5rem]">
              ...
              <div className="flex flex-col gap-[0.5rem]">
                {[
                  ...
                  <div key={label} className="flex items-center justify-between bg-surface-card dark:border dark:border-surface-border rounded-2xl card-shadow px-[1rem] py-[0.75rem]">
                    <div className="flex items-center gap-[0.5rem]">
      ...
      <AnimatePresence>
        {selectedIds.size > 0 && (
          <motion.div variants={slideInBottom} initial="hidden" animate="visible" exit="exit" className="fixed bottom-[3.5rem] left-1/2 -translate-x-1/2 z-50">
            <div className="bg-surface-elevated border border-surface-border rounded-2xl shadow-2xl px-[1.25rem] py-[0.875rem] flex items-center gap-[1rem]">
            <span className="text-ink-secondary text-[0.875rem] font-medium whitespace-nowrap">
              {selectedIds.size} selecionada{selectedIds.size !== 1 ? 's' : ''}
            </span>
            <div className="w-[1px] h-[1.25rem] bg-surface-border" />
            <button
              onClick={bulkConcluir}
              className="flex items-center gap-[0.375rem] text-[0.8125rem] font-medium text-status-green hover:opacity-80 transition-opacity"
            >
            ...
            <button
              onClick={bulkAdiar}
              className="flex items-center gap-[0.375rem] text-[0.8125rem] font-medium text-ink-secondary hover:text-ink-primary transition-colors"
            >
            ...
            <button
              onClick={bulkDeletar}
              className="flex items-center gap-[0.375rem] text-[0.8125rem] font-medium text-status-red hover:opacity-80 transition-opacity"
            >
      ...
      <AnimatePresence>
        {modalAberto && (
          <TaskModal
            ...
            onSaved={() => { viewMode === 'kanban' ? carregarKanban() : carregar() }}
          />
        )}
      </AnimatePresence>
    </MainLayout>
